...
routes.put("/editar/:id", async (req, res) => {
  try {
    ...
    const mantenimeintoEditado = await MantenimientoModel.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
    ...
    else return res.status(201).json(mantenimeintoEditado);
  } catch (error) {
    res.status(400).json({ mensaje: error.message });
  }
});
...
routes.delete("/eliminar/:id", async (req, res) => {
  try {
    const mantenimeintoeliminado = await MantenimientoModel.findByIdAndDelete(
      req.params.id
    );
    if (!mantenimeintoeliminado)
      return res.status(404).json({ mensaje: "Mantenimiento no encontraod" });
    else return res.json({ mensaje: "Mantenimiento Elimionado!!!" });
  } catch (error) {
    res.status(500).json({ mensaje: error.message });
  }
});
//eliminar todos
routes.delete("/eliminartodos", async (req, res) => {
  try {
    await MantenimientoModel.deleteMany();
    return res.json({
      mensaje: "Todos los mantenimeintos han sido eliminados",
    });
  } catch (error) {
    res.status(500).json({ mensaje: error.message });
  }
...
routes.get("/totalmantenimiento", async (req, res) => {
  try {
...
routes.get("/diaz/:usuario_peticion", async (req, res) => {
    console.log("Usuario petición:", req.params.usuario_peticion);
  try {
    const mantenimiento_peticion = await MantenimientoModel.find({
      usuario_peticion: new RegExp(req.params.usuario_peticion, "i"),
    });
...
routes.get("/mantenimiento/:id", async (req, res) => {
  try {
    ...
    else return res.json(mantenimiento);
  } catch (error) {
    res.status(500).json({ mensaje: error.message });
  }
});
...
routes.get("/ordenarmantenimientos", async (req, res) => {
  try {
    const ordenmantenimiento = await MantenimientoModel.find().sort({
      nombre_tecnico: -1,
    });
...
//REPORTE UNO
routes.get(
  "/mantenimientoporusuario/:usuarioId",
...
routes.get("/mantenimientosumatecnico", async (req, res) => {
  try {
